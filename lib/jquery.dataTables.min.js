...
["sScrollY","sY"],["bScrollCollapse","bCollapse"]]);D(k.oLanguage,g,"fnInfoCallback");x(k,"aoDrawCallback",g.fnDrawCallback,"users");x(k,"aoServerParams",g.fnServerParams,"users");x(k,"aoStateSaveParams",g.fnStateSaveParams,"users");x(k,"aoStateLoadParams",g.fnStateLoadParams,"users");x(k,"aoStateLoaded",g.fnStateLoaded,"users");x(k,"aoRowCallback",g.fnRowCallback,"users");x(k,"aoRowCreatedCallback",g.fnCreatedRow,"users");x(k,"aoHeaderCallback",g.fnHeaderCallback,"users");x(k,"aoFooterCallback",g.fnFooterCallback,
"users");x(k,"aoInitComplete",g.fnInitComplete,"users");x(k,"aoPreDrawCallback",g.fnPreDrawCallback,"users");n=k.oClasses;g.bJQueryUI?(h.extend(n,p.ext.oJUIClasses,g.oClasses),g.sDom===m.sDom&&"lfrtip"===m.sDom&&(k.sDom='<"H"lfr>t<"F"ip>'),k.renderer)?h.isPlainObject(k.renderer)&&!k.renderer.header&&(k.renderer.header="jqueryui"):k.renderer="jqueryui":h.extend(n,p.ext.classes,g.oClasses);h(this).addClass(n.sTable);if(""!==k.oScroll.sX||""!==k.oScroll.sY)k.oScroll.iBarWidth=Gb();!0===k.oScroll.sX&&(k.oScroll.sX=
